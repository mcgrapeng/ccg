...
| 시나리오 | 사용? | 이유 |
|---|---|---|
| auth / 결제 / 마이그레이션 / 암호화 변경 | ✅ 예 | 다른 훈련 데이터가 다른 버그를 잡음. $0.04 값어치. |
| 솔로 개발 / 2인 팀, 두 번째 리뷰어 없음 | ✅ 예 | "또 다른 한 쌍의 눈"에 가장 가까운 것 |
| 200줄 PR 머지 전 마지막 점검 | ✅ 예 | 위험 라우터가 적절한 모델 자동 선택 |
| 변수 이름 변경 | ❌ 아니요 | 그냥 commit |
| 문서만 편집 | ❌ 아니요 | 위험 라우터가 ~$0.0007로 자동 다운그레이드하지만 진짜 불필요 |
| 단일 모델과 스트리밍 채팅하고 싶음 | ❌ 아니요 | codex / gemini CLI 직접 사용 |
